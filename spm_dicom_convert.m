...
% $Id: spm_dicom_convert.m 36 2012-08-28 11:42:20Z gaser $
...
for i=1:length(hdr),
		tmp=hdr{i}{1};
		dirname=fileparts(tmp.Filename);
		[tmp prename]=fileparts(dirname);
    fnames{i} = write_volume(hdr{i},root_dir,format);
    [pth nam ext] = fileparts(fnames{i}); 
    fprintf('rename %s to %s\n',fnames{i},[prename '_' nam ext]);
    if exist(fullfile(pth,[nam '.hdr']),'file')
      movefile(fullfile(pth,[nam '.hdr']),[prename '_' nam '.hdr']);
      movefile(fullfile(pth,[nam '.img']),[prename '_' nam '.img']);
    else 
      movefile(fullfile(pth,[nam '.nii']),[prename '_' nam '.nii']);
    end
end;
return;
%_______________________________________________________________________
