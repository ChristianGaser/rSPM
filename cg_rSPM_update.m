function cg_rSPM_update(update)
% check for new updates
%
% FORMAT cg_rSPM_update(update)
% update - allow installation of update
% 
% This function will connect itself to the SBM server, compare the
% version number of the updates with the one of the rSPM installation 
% currently in the MATLAB path and will display the outcome.
%_______________________________________________________________________
% Christian Gaser
% $Id: cg_rSPM_update.m 35 2012-08-28 11:22:47Z gaser $
...
url = 'http://dbm.neuro.uni-jena.de/rSPM/';
...
n = regexp(s,'rSPM_r(\d.*?)\.zip','tokens');
if isempty(n)
...
if rnew > r
  fprintf('A new version of rSPM is available on: %s\n',url);
  fprintf('Your version: %d - New version: %d\n',r,rnew);
  if ~update
    fprintf('In order to update use Toolbox|rSPM|Check for updates\n',r,rnew);
  end

  if update
    d = fullfile(spm('Dir'),'toolbox'); 
    overwrite = spm_input('Update',1,'m','Do not update|Download zip-file only|Overwrite old rSPM installation',[-1 0 1],3);
    switch overwrite
    case 1
      try
        % list mex-files and delete these files to prevent that old
        % compiled files are used
        mexfiles = dir(fullfile(d,'rSPM','*.mex*'));
        for i=1:length(mexfiles)
          name = fullfile(d,'rSPM',mexfiles(i).name);
          spm_unlink(name);
        end
        fprintf('Download rSPM\n');
        s = unzip([url sprintf('rSPM_r%d.zip',rnew)], d);
        ...
        web([url sprintf('rSPM_r%d.zip',rnew)],'-browser');
