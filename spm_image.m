...
if strcmp(op,'zoom_in'),
	...
	res  = mean(svd(st.vols{1}.mat(1:3,1:3)));

	if op==1,
		spm_orthviews('addcolouredblobs',0.2);
		spm_orthviews('MaxBB');
		spm_orthviews('resolution',res);
    else
		...
		pos = st.Space\[pos ; 1];
		pos = pos(1:3)';
		if     op == 2, st.bb = [pos-8*vx ; pos+8*vx] ; spm_orthviews('resolution',res);
		elseif op == 3, st.bb = [pos-4*vx ; pos+4*vx] ; spm_orthviews('resolution',res/2);
		elseif op == 4, st.bb = [pos-2*vx ; pos+2*vx] ; spm_orthviews('resolution',res/4);
		elseif op == 5, st.bb = [pos-1*vx ; pos+1*vx] ; spm_orthviews('resolution',res/8);
        else            st.bb = [pos-.5*vx ; pos+.5*vx] ; spm_orthviews('resolution',res/8);
		end;
	end;
	return;
end;
...
st.zoomer = uicontrol(fg,'Style','popupmenu' ,'Position',[315 75 125 20].*WS,...
	'String',char('Full Volume','16x16x16mm','8x8x8mm','4x4x4mm','2x2x2mm','1x1x1mm'),...
