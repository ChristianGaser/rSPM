...
item1_2   = uimenu(item1,      'Label','16x16x16mm',    'Callback','spm_orthviews(''context_menu'',''zoom'',5);');
item1_3   = uimenu(item1,      'Label','8x8x8mm',       'Callback','spm_orthviews(''context_menu'',''zoom'',4);');
item1_4   = uimenu(item1,      'Label','4x4x4mm',       'Callback','spm_orthviews(''context_menu'',''zoom'',3);');
item1_5   = uimenu(item1,      'Label','2x20x2mm',      'Callback','spm_orthviews(''context_menu'',''zoom'',2);');
item1_6   = uimenu(item1,      'Label','1x1x1mm',       'Callback','spm_orthviews(''context_menu'',''zoom'',1);');
...
global st
cm_handles = get_cm_handles;
r  = mean(svd(st.Space(1:3,1:3)));
res = [r/8 r/8 r/4 r/2 r r];
if op==6,
	st.bb = maxbb;
else
	...
	pos = st.Space\[pos ; 1];
	pos = pos(1:3)';
	if     op == 5, st.bb = [pos-8*vx ; pos+8*vx] ;
	elseif op == 4, st.bb = [pos-4*vx ; pos+4*vx] ;
	elseif op == 3, st.bb = [pos-2*vx ; pos+2*vx] ;
	elseif op == 2, st.bb = [pos-1*vx ; pos+1*vx] ;
